...
def get_data(json_record):
    ...
    try:
            ...
            receiver_id=int(json_body['transactions']['target']['id'])
            ...
            return ((year,month,date,hour,dayofweek,sender_id,receiver_id), 1)
    except:
            return None
...
data_schema4="""CREATE TABLE IF NOT EXISTS Data_2015_Ver1_user2 (
                hour INT(2),
                receiver_id INT,
                count INT,
                PRIMARY KEY (hour, receiver_id)
                );
                """
...
stmt4=""" insert ignore into Data_2015_Ver1_user2 (hour, receiver_id,count) VALUES (%s,%s,%s);"""
...
def write_user2_data(partition):
    connection=mysql.connector.connect(**db_config)
    ...
    for x in partition:
        ...
        hour=x[0][0]
        count=x[1]
        data.append(hour,receiver_id,count)
        results.append(count)
    try:
        cursor.executemany(stmt4,data)
        connection.commit()
    except:
        connection.rollback()
    cursor.close()
...
if __name__ == "__main__":
    ...
    data_rdd_hour_user2=data_rdd.map(lambda rdd:((rdd[0][6],rdd[0][3]),rdd[1]))reduceByKey(lambda a,b:a+b)
    ...
    if table_create4:
        written_entries=data_rdd_hour_user.mapPartitions(write_user2_data)
    else:
        print('cannot create table by hour and user2')
        sys.exit(0)
